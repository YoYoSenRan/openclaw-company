---
name: "OPSX: Apply"
description: 实现一个 OpenSpec 变更的任务（实验性）
category: Workflow
...
**输入**：可选择指定一个变更名称（例如 `/opsx:apply add-auth`）。如果省略，检查是否可以从对话上下文中推断。如果模糊或不明确，你必须提示用户选择可用的变更。
...
2. **检查状态以了解模式**
   ...
   - 哪个工件包含任务（对于 spec-driven 通常是 "tasks"，其他情况请检查状态）
...
- **可随时调用**：在所有工件完成之前（如果任务存在）、部分实现之后、与其他操作交替进行
- **允许工件更新**：如果实现发现设计问题，建议更新工件 - 不是阶段锁定的，灵活工作
